...
static void on_tx_done(uint32_t tx_size, void *user_arg)
{
    ESP_EARLY_LOGI(TAG, "TX complete: %lu bytes", (unsigned long)tx_size);
}

static void on_rx_done(const uint8_t *data, uint32_t size, void *user_arg)
{
    ESP_EARLY_LOGI(TAG, "RX complete: %lu bytes, echoing back...", (unsigned long)size);

    // user_arg points to the global handle variable so we can dereference it here
    esp_isotp_handle_t handle = NULL;
    if (user_arg) {
        handle = *(esp_isotp_handle_t *)user_arg;
    }
...
static esp_err_t isotp_echo_init(void)
{
    ...
    esp_isotp_config_t isotp_cfg = {
        ...
        // Pass callbacks via config in the new API
        .rx_callback = on_rx_done,
        .tx_callback = on_tx_done,
        // Pass the address of the global handle so callback can dereference it
        .callback_arg = &g_isotp_handle,
    };
