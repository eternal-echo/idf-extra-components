 ...
 *     .tx_id = 0x123, .rx_id = 0x124,
 *     .tx_buffer_size = 4095, .rx_buffer_size = 4095,
 *     .use_extended_id = false,
 *     .rx_callback = NULL,  // Polling mode
 *     .tx_callback = NULL,
 *     .callback_arg = NULL
 * };
 *
 ...
 * ```
 *
 * ## Event-Driven Usage (Callback Mode)
 * ```c
 * void my_rx_callback(const uint8_t *data, uint32_t size, void *user_arg) {
 *     // Process received data - copy immediately if needed beyond callback
 *     printf("Received %d bytes\n", size);
 *     // Handle received data...
 * }
 *
 * void my_tx_callback(uint32_t tx_size, void *user_arg) {
 ...
 *     .tx_buffer_size = 4095, .rx_buffer_size = 4095,
 *     .use_extended_id = false,
 *     .rx_callback = my_rx_callback,  // Event-driven mode
 *     .tx_callback = my_tx_callback,
 *     .callback_arg = &my_context
 * };
 *
 * esp_isotp_handle_t handle;
 * esp_isotp_new_transport(twai_node, &config, &handle);
 *
 * while (1) {
 *     esp_isotp_poll(handle);  // MUST still call regularly!
 *     vTaskDelay(pdMS_TO_TICKS(5));
 * }
 * ```
 */
 ...
 * @param out_handle Returned ISO-TP transport handle.
 * @return
 *  - ESP_OK on success
